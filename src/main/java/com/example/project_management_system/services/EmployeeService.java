...
public class EmployeeService {
  ...
  // Returns the raw entity — for internal use by other services that need the
  // entity, not the DTO
  @Transactional(readOnly = true)
  ...
  // Finds an employee by email for registration — throws ConflictException if not
  // provisioned
  @Transactional(readOnly = true)
  ...
  // Links a user account to an employee — explicit save needed as no cascade from
  // User to Employee
  @Transactional
  ...
  public void deleteById(Long id) {
    ...
    Employee employee = employeeRepository.findById(id).orElseThrow(() -> ResourceNotFoundException.employee(id));
