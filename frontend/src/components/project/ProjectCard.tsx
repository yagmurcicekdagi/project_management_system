...
export function ProjectCard({
  ...
  const { data: assignmentData = [] } = useProjectAssignments(
    Number(project.id),
  );
  ...
  const progress = project.status
    ? (STATUS_PROGRESS[project.status] ?? null)
    : null;
  ...
  return (
    ...
    >
      <CardContent className="flex flex-col flex-1 p-5 gap-4">
        ...
        {dueStr && (
          <div
            className={`inline-flex items-center gap-1.5 self-start text-xs px-3 py-1.5 rounded-xl font-medium ${
              isOverdue
          ...
          >
            ...
            Due:{" "}
            {new Date(dueStr).toLocaleDateString("en-US", {
              month: "long",
              day: "numeric",
              year: "numeric",
            })}
