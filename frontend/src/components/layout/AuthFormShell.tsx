    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 bg-white flex items-center justify-center px-8 py-12">
        <div className="w-full max-w-md">
          <h2 className="text-3xl font-bold tracking-tight mb-1 animate-fade-up [animation-delay:0ms]">
            {title}
          </h2>
          <p className="text-gray-500 mb-8 animate-fade-up [animation-delay:80ms]">
            {subtitle}
          </p>
          ...
          <div className="flex rounded-xl border border-gray-200 overflow-hidden mb-8 animate-fade-up [animation-delay:160ms]">
            {activeTab === "signin" ? (
              <>
                ...
                <Link to="/register" className={`${tabBase} ${tabInactive}`}>
                  Sign Up
                </Link>
              </>
            ) : (
              <>
                <Link to="/login" className={`${tabBase} ${tabInactive}`}>
                  Sign In
                </Link>
